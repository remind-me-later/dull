...
isAssociative :: BinOperator -> Bool
isAssociative OrOp = False
isAssociative AndOp = False
isAssociative _ = True
...
showExprInnerWithContext parentPrec isRightSide (BinExpr left op right) =
  let myPrec = precedence op
      ...
      needsParens = assoc && (myPrec < parentPrec || (myPrec == parentPrec && isRightSide))
